...
const Header = () => {
  ...
  return (
    <>
        <header className="bg-[#E6DFC3aa] shadow-sm sticky top-0 z-10">
    <div className="max-w-8xl mx-auto px-2 sm:px-4 lg:px-6 py-2">
      <div className="flex justify-between items-center  md:justify-start md:space-x-10">
        <div className="flex justify-start lg:w-0 lg:flex-1">
          <a href="/" className="flex items-center">
            <img src={AppImages.logo} className='h-32' alt="" />
          </a>
        </div>
        ...
        <nav className="hidden md:flex space-x-10">
          {sidebarItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="text-xl font-medium text-green-500 hover:text-green-800"
            >
              {item.label}
            </a>
          ))}
